...
[![Travis-CI Build Status](https://travis-ci.com/tlverse/tmle3mediate.svg?branch=master)](https://travis-ci.com/tlverse/tmle3mediate)
...
__Authors:__ [Nima Hejaz](https://nimahejazi.org), James Duncan, David McCoy,
and [Mark van der Laan](https://vanderlaan-lab.org)
...
which estimation machinery are provided include the controlled direct effect
[@petersen2006estimation; @didelez2012direct; @vanderweele2015explanation], the
natural (in)direct effects [@robins1992identifiability; @zheng2012targeted], and
the population intervention (in)direct effects [@diaz2020causal]. By building on
the core `tlverse` grammar exposed by the `tmle3` R package, `tmle3mediate`
accommodates targeted maximum likelihood (or targeted minimum loss-based)
estimation of these causal effect parameters through a unified interface. For a
general discussion of the framework of targeted minimum loss-based estimation
and its relationship to statistical causal inference, the motivated reader may
consider consulting @vdl2011targeted and @vdl2018targeted. A practical and
accessible introduction using the `tlverse` software ecosystem is provided in
@vdl2021targeted (see https://tlverse.org/tlverse-handbook).
...
* [R/`medshift`](https://github.com/nhejazi/medshift) - An R package providing
  ...
  the mediation setting, including classical (IPW) and doubly robust one-step
  estimators. This is an implementation of the methodology explored
  by @diaz2020causal.
...
* [R/`medoutcon`](https://github.com/nhejazi/medoutcon) - An R package providing
  ...
  effects, which are defined by joint static-stochastic interventions applied
  to the exposure and mediators, respectively. These effect definitions are
  similar to but more general than the natural (in)direct effects. This is an
  implementation of the methodology explored by @diaz2020nonparametric.
