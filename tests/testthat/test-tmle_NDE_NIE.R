...
# create continuous Y and binary Y data and set column names for sl3 tasks (for convenience)
...
tmle_fit_NIE_cont_y <- run_NIE(data = data_cont_y, binary_outcome = FALSE, max_iter = 10)
tmle_fit_NIE_binary_y <- run_NIE(data = data_binary_y, binary_outcome = TRUE, max_iter = 200)


## one-line call with faster with tmle3 wrapper
set.seed(71281)

## instantiate tmle3 spec for NIE outside of function
...
## run the above function for continuous and binary outcomes
tmle_fit_NDE_cont_y <- run_NDE(data = data_cont_y, binary_outcome = FALSE, max_iter = 10)
tmle_fit_NDE_binary_y <- run_NDE(data = data_binary_y, binary_outcome = TRUE, max_iter = 200)
...
# simulate data and extract components for computing true parameter value for binary and continuous outcome
sim_truth_cont_y <- get_sim_truth_NIE_NDE(n_obs = 1e7, binary_outcome = FALSE, EIC = FALSE)
sim_truth_binary_y <- get_sim_truth_NIE_NDE(n_obs = 1e7, binary_outcome = TRUE, EIC = FALSE)
