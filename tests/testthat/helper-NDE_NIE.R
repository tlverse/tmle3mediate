get_sim_truth_NIE_NDE <- function(n_obs = 1e7, EIC = FALSE) {
  ...
  if (EIC) {
    ...
    Y <- dgp$m_mech(W, A, Z)
    Qbar_Y <- dgp$m_mech(W, A, Z, eps_sd = 0)
    ...
    HY <- A * Q_Z_Ais0 / Q_Z_Ais1 / g1 - (1 - A) / g0
    HZ <- 1 / g0
    ...
    EIC_NDE <- D_Y_NDE + D_Z_NDE + D_W_NDE - NDE

    ###################################################
    ## NIE

    EIC_NIE <- (A / g1) * (
      Y - EY_A1_Z1 - Q_Z_ratio * (Y - m_Ais1)
