...
binary_learner_lib <- Stack$new(
  ...
  fglm_binary_learner, mean_learner
)
...
The process of estimating the NDE begins by constructing $\hat{\bar{Q}}_Y$, an
estimate of the outcome mechanism $\bar{Q}_Y(Z, A, W)$ (i.e., the conditional
mean of $Y$, given $Z$, $A$, and $W$). With an estimate of this conditional
expectation in hand, predictions of the counterfactual quantities $\bar{Q}_Y(Z,
1, W)$ (setting $A = 1$) and, likewise, $\bar{Q}_Y(Z, 0, W)$ (setting $A = 0$)
can easily be obtained. We denote the difference of these counterfactual
quantities $\bar{Q}_{\text{diff}}$, i.e., $\bar{Q}_{\text{diff}} = \bar{Q}_Y(Z,
1, W) - \bar{Q}_Y(Z, 0, W)$. $\bar{Q}_{\text{diff}}$ represents the difference
in $Y$ attributable to changes in $A$ while keeping $Z$ and $W$ at their natural
(observed) values.

The estimation procedure treats $\bar{Q}_{\text{diff}}$ itself as a nuisance
parameter, regressing its $\hat{\bar{Q}}_{\text{diff}}$ estimate on $W$, among
...
removed during the TML estimation step using an auxiliary (or "clever")
covariate based on the mediators $Z$. This auxiliary covariate take the form
\begin{equation*}
...
conditional means, opening the door to the use of a wide range of machine
learning algorithms.
...
= 1$ given $Z$ and $W$, are used in constructing the auxiliary covariate for TML
...
Derivation and estimation of the NIE is analogous to that of the NDE.
The NIE is the effect of $A$ on $Y$ _through the mediator(s) $Z$_.
This quantity -- known as the additive natural indirect effect $E(Y(Z(1), 1) -
E(Y(Z(0), 1)$ -- corresponds to the difference of the conditional expectation of
$Y$ given $A = 1$ and $Z(1)$ (the values the mediator would take under $A = 1$)
and the conditional expectation of $Y$ given $A = 1$ and $Z(0)$ (the values the
...
altogether. However, now, the mediated mean outcome difference, denoted
...
treated units (for whome $A = 1$ is observed) to obtain the conditional mean
...
on $Z$. So, in the case of the NIE, our $\Psi$ estimate is slightly different,
but the same quantity $e(A \mid Z, W)$ is used as the auxiliary covariate.
...
3. We then use clever covariates (like the ones described above) in a
   parametric sub-model through the initial fit of $\hat{Q}^0_n(A,W)$, and
...
the observed $Y$ as outcome, offset by the initial estimates $\hat{Q}^0_n(A,W)$
and the auxiliary covariate as independent variable. Once fit, the coefficient
is used to update the initial predictions $\hat{Q}^0_n(A,W)$ to make
...
instantiating a "Spec" object that encodes

which learners to use for the nuisance parameters $e(A \mid Z, W)$ and
$\Psi_Z$. We then pass our Spec object onto the `tmle3` function, alongside the
data, the node list (created above), and a learner list indicating which machine
learning algorithms to use for estimating the nuisance parameters based on $A$
and $Y$.
...
The same procedure is done to get the NDE, only replacing the Spec object for
the NIE with `tmle_spec_NDE` to define learners for the NDE nuisance parameters:
...
At times, the natural direct and indirect effects can be too limiting, as they
are based on _static interventions_ (i.e., setting $A = 0$ or $A = 1$), which
may be unrealistic for real-world interventions. In such cases, one may turn
instead to the population intervention direct effect (PIDE) and the population
intervention indirect effect (PIIE), which based on decomposing the effect of
the population intervention effect (PIE) based on flexible stochastic
interventions [@diaz2020causal].
...
* for $\mathbb{E}\{Y(A_{\delta}, Z)\}$, an efficient one-step estimator for the
  effect of a joint intervention altering the treatment mechanism but not the
  mediation mechanism, as proposed in @diaz2020causal; and,
* for $\mathbb{E}\{Y(A_{\delta}, Z_{A_{\delta}})\}$, an efficient one-step
  estimator for the effect of a joint intervention altering both the treatment
...
The efficient
one-step estimator may be computed using the EIF estimating equation and may
incorporate cross-validation [@zheng2011cross; @chernozhukov2018double] to
circumvent restrictive entropy conditions (e.g., Donsker class). The resultant
estimator is
\begin{equation*}
...
\end{equation*}
which is implemented in the [`medshift` R
package](https://github.com/nhejazi/medshift). Using the EIF, a _targeted
maximum likelihood_ estimator may also be formulated.  We demonstrate the use of
`tmle3mediate` to obtain $\mathbb{E}\{Y(A_{\delta}, Z)\}$ via its TML estimator:
